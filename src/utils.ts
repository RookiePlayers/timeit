...
export class Utils {
  ...
  // ── timers / status
  private startTimers() {
    const idleMinutes = this.vscode.workspace.getConfiguration().get<number>('timeit.idleTimeoutMinutes') ?? 5;
  ...
  notify(msg: string, type: 'info' | 'warn' | 'error' = 'info') {
    const show = this.vscode.workspace.getConfiguration().get<boolean>('timeit.showNotifications') ?? true;
    if (!show) {return;}
